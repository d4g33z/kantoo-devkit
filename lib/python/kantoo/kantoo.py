...
class PluginConfig:
    ...
    def __init__(self,script_pwd,config_rel_path):
        ...
        self.env_plugins = [EnvPluginBase(var, value) for var, value in self.config.get('envplugins', {}).items()]
...
class Config:
    ...
    def __init__(self,script_pwd,config_rel_path):
        ...
        self.env_plugins = [EnvPluginBase(var, value) for var, value in self.config.get('envplugins', {}).items()]
        self.dir_plugins = [DirPluginBase(**value) for value in self.config.get('dirplugins', {}).values()]

        #bind the contents of DOT_DIR as hidden files and dirs in /root
        if hasattr(self,'DOT_DIR') and pathlib.Path(os.path.join(self.SCRIPT_PWD,self.DOT_DIR)).exists():
            dot_dir_configs,dot_file_configs = self._dot_plugins(self.DOT_DIR)
            [self.dir_plugins.append(DirPluginBase(**dirplugin_values)) for dirplugin_values in dot_dir_configs.values()]
            [self.file_plugins.append(FilePluginBase(**fileplugin_values).write(open(os.path.join(self.SCRIPT_PWD, fileplugin_values.get('path'))).read(), '')) for fileplugin_values in dot_file_configs.values()]
    ...
    def _exec_or_file_plugins(self, type):
        ...
        return list(map(lambda x,y,z,a:x.write(y,a,**z),
                        #create the objs
                        [FilePluginBase(**pluginblock.get(x)) if type == 'fileplugins' else ExecPluginBase(x, **pluginblock.get(x)).chmod(0o744) for x in pluginblock.keys()],
                        #get the text from the hjson file or a file on disk
...
class EnvPluginBase(PluginBase):
...
class DirPluginBase(PluginBase):
...
class ExecPluginBase(PluginBase):
...
class FilePluginBase(PluginBase):
